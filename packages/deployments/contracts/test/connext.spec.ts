...
describe("Connext", () => {
  ...
  describe("top level batching", () => {
    it.only("should work for tokens", async () => {
      ...
      // Fulfill with the router
      const routerAmount = amount - 500;
      let multicallData = nonces.map( nonce => {
        return destinationTm.interface.encodeFunctionData(
          'execute',
          [{
            params,
            nonce,
      ...
      const execute = await destinationTm.connect(router).multicall(multicallData);
      const execReceipt = await execute.wait();
